...
title_label = ctk.CTkLabel(menu_frame, text="Film Flow", text_color="white", font=("morganite", 22, "bold"))
title_label.place(x=12, y=10)

search_bar = ctk.CTkEntry(menu_frame, placeholder_text="Search", placeholder_text_color="black",
...
def on_movie_select(frame, movie_data):
    ...
    # Reset the appearance of previously selected movie
    if selected_movie_frame:
        selected_movie_frame.configure(border_width=0, fg_color="#333333")

    # Highlight the newly selected movie
    frame.configure(border_width=2, border_color="#E50914", fg_color="#444444")
    selected_movie_frame = frame

    # You can now use movie_data for whatever you need
    print(f"Selected movie: {movie_data['title']}")
    # Here you could show details, play trailer, etc.


# Create movie sections with selectable frames
